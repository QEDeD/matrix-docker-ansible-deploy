...
def docker_summary_display_state(display_state: Any) -> Dict[str, Any]:
    raw = _coerce_to_mapping(display_state, {})
    versions = _coerce_to_mapping(raw.get("versions"), {})
    metadata = _coerce_to_mapping(raw.get("metadata"), {})
    displayed_at_raw = raw.get("displayed_at")
    displayed_at = "" if displayed_at_raw is None else str(displayed_at_raw).strip()
    return {
        "versions": versions,
        "metadata": metadata,
...
class FilterModule(object):
    def filters(self) -> Dict[str, Any]:
        return {
            ...
            "docker_summary_display_state": docker_summary_display_state,
