...
./roles/docker_ansible_summary/tests/run-local-suite.sh
```
...
```bash
.venv/bin/ansible-playbook roles/docker_ansible_summary/tests/local_suite.yml \
...
| `recovery-visibility` | Validates that diffs are computed from last displayed state and shows a recovery notice when unseen changes are replayed. |
...
- **`ERROR! the role 'docker_ansible_summary' was not found`** – Run the
