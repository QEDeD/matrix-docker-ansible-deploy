...
Use the repository lint entrypoint so scope handling stays consistent:
...
yamllint roles/docker_ansible_summary
```

## Targeted Playbook Tests

Most regression coverage lives under `roles/docker_ansible_summary/tests`. Useful subsets:

```bash
# Change-detection regression (scope changes, removals, etc.)
ansible-playbook roles/docker_ansible_summary/tests/test_scope_change.yml

# Filter helper smoke tests
ansible-playbook roles/docker_ansible_summary/tests/test_scope_filters.yml

# BSD retention fallback (date -v)
ansible-playbook roles/docker_ansible_summary/tests/test_bsd_retention.yml

# Mock mode table rendering
ansible-playbook roles/docker_ansible_summary/tests/test_mock_mode.yml

# History renderer smoke test
ansible-playbook roles/docker_ansible_summary/tests/test_history.yml

# Notes column + auto width
ansible-playbook roles/docker_ansible_summary/tests/test_table_notes.yml
```

Each play sets:
- `docker_ansible_summary_enable_discovery: false` – avoids Docker calls.
- `docker_ansible_summary_write_facts: false` – prevents filesystem writes.
...
```bash
cd /home/ansible-admin/matrix-docker-ansible-deploy/roles/docker_ansible_summary
...
for play in \
  ...
  ansible-playbook roles/docker_ansible_summary/tests/$play || break
done
```
...
|---------|--------------|------------|
| `ModuleNotFoundError: ansible.parsing.yaml.constructor` | Running system `ansible-lint` without `ansible-core` modules | Run `bash ./bin/lint-playbook.sh` (it prepares `.venv`) or install `ansible-core` system-wide. |
...
| Tests try to write to `/etc/ansible/facts.d/` | The play or include role sets `docker_ansible_summary_write_facts: true` | Override to `false` in the test vars or include helper tasks from `tests/tasks/`. |

## Workflow Checklist

1. Update docs (`docs/WORKLOG.md`, `docs/DESIGN.md`, this file) when behaviour changes.
2. Run lint: `LINT_PLAYBOOK_ROLE_PATHS="roles/docker_ansible_summary" bash ./bin/lint-playbook.sh`.
