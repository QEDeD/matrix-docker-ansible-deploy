...
- Compares current state against the last successfully displayed baseline (with fallback bootstrap from collected snapshots) to classify services as baseline/added/updated/removed.
...
┌─────────────────────────────────────────────────────────────────────┐
│ tasks/main.yml                                                      │
│  ├─ mock_mode.yml (optional, toggled via docker_ansible_summary_mock_mode)  │
│  ├─ display_summary.yml (primary execution path)                    │
│  └─ view_history*.yml (invoked when docker_ansible_summary_show_history)    │
└─────────────────────────────────────────────────────────────────────┘
```
...
- `docker_summary_display_state` normalises the persisted "last displayed" baseline facts.
- `docker_ansible_summary_container_*` parse names/images from Docker inspect responses.
...
`docker_ansible_summary_enable_discovery` and `docker_ansible_summary_write_facts` default to `true` but can be disabled for offline or local tests. The helper include files in `tests/tasks/` set them to false along with standardising `ansible_date_time`/`ansible_system`.
...
- All intermediary tasks (`set_fact`, retention calculations, history bookkeeping) respect `docker_ansible_summary_quiet_tasks` by default to avoid log spam.
- History rendering is opt-in (`docker_ansible_summary_show_history`) so production runs only display the single summary block unless explicitly requested.
- A dedicated action plugin writes the table directly to stdout to avoid JSON-escaped `\n` artifacts in the default callback.
- If DAS detects that collected snapshots advanced in a run where output never displayed, the next successful run prints a recovery footer to explain why old-looking changes are still shown.
...
### 6. Run-Aware Baseline and Commit Ordering
DAS keeps two distinct baselines:
- **Collected baseline** (`docker_ansible_summary_versions_fact_file` + `last_metadata` in history): latest discovered state.
- **Displayed baseline** (`docker_ansible_summary_display_state_fact_file`): the state actually shown to operators.

Diffs are computed from the displayed baseline. This keeps output aligned with operator experience after mid-run failures.

Persistence order in `display_summary.yml` is intentional:
1. Render and display summary output.
2. Mark output as rendered.
3. Write displayed baseline fact.
4. Write collected versions fact.
5. Write history fact.

This ordering prevents advancing the displayed baseline unless output was successfully emitted.

### 7. Fact Storage Contract
The role reads and writes JSON facts under `/etc/ansible/facts.d/` (`docker_ansible_summary_versions_fact_file`, `docker_ansible_summary_history_fact_file`, `docker_ansible_summary_display_state_fact_file`). Filenames remain customisable, but the JSON schema is:
```json
{
  "service": {
    "current": "...",
...
```json
{
  ...
  "metadata": {
    "service": { "image": "...", "state": { "status": "running" } }
  },
  "displayed_at": "2026-02-16T12:00:00Z"
}
```
...
- `docker_ansible_summary_scope`: glob(s) controlling which containers are included.
- `docker_ansible_summary_display`, `docker_ansible_summary_show_history`: toggle output paths.
- `docker_ansible_summary_history_max_entries`, `docker_ansible_summary_retention_days`: enforce retention.
- `docker_ansible_summary_display_state_fact_file`: choose where the last displayed baseline is persisted.
- `docker_ansible_summary_table_*`: configure column widths, opt into adaptive sizing, switch Unicode/ASCII borders, and expose the optional NOTES column.
- `docker_ansible_summary_container_overrides`: inject test data instead of real discovery.
