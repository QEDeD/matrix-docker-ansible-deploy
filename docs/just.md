...
When running with `setup-*` tags together with a start or restart tag (e.g. `setup-all,start` or `setup-synapse,start-group`), all targeted services are unconditionally restarted regardless of whether changes were detected. This is appropriate for setup's thorough "full setup" semantics.
