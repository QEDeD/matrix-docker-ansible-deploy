...
- `setup-SERVICE` (e.g. `setup-postmoogle`) - runs setup tasks only for a given
  ...
  other. For example, setting up the
  [mautrix-telegram bridge](configuring-playbook-bridge-mautrix-telegram.md),
  in addition to `setup-mautrix-telegram`, usually requires database changes
  (`setup-postgres`) and reverse-proxy changes (`setup-nginx-proxy`).

- `install-SERVICE` (e.g. `install-postmoogle`) - like `setup-SERVICE`, but
...
- `ensure-matrix-users-created` (alias: `ensure-users-created`) - ensures all
  special users needed by the playbook (bots, etc.) are created. See
  `matrix_user_creator_users_auto` in `group_vars/matrix_servers` for defaults.
...
- `just install-all` expands to
  `install-all,ensure-matrix-users-created,start`, and `just setup-all`
  expands to `setup-all,ensure-matrix-users-created,start`.
